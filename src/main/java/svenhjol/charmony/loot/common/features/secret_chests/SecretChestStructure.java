...
public class SecretChestStructure extends Structure {
    ...
    private Optional<BlockPos> findStart(GenerationContext context) {
        var stoneChestsEnabled = Mod.tryGetFeature(ResourceLocation.parse("charmony-stone-chests:stone_chests")).map(Feature::enabled).orElse(false);

        if (!feature().enabled() || !stoneChestsEnabled) {
            return Optional.empty();
        }
